...
class HuXiu(object):
    ...
    def visit_index(self):
        ...
        reg_element = self.wait.until(EC.element_to_be_clickable((By.CLASS_NAME, 'js-register')))
        reg_element.click()

        self.wait.until(EC.element_to_be_clickable((By.CLASS_NAME, 'gt_slider_knob gt_show')))
        # 进入模拟拖动流程
        self.analog_drag()

    def analog_drag(self):
        # 鼠标移动到拖动按钮，显示出拖动图片
        element = self.wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'gt_slider_knob gt_show')))
        ActionChains(self.driver).move_to_element(element).perform()
        time.sleep(3)

        # 刷新一下极验图片
        element = self.wait.until(EC.presence_of_element_located((By.CLASS_NAME, 'gt_refresh_button')))
        element.click()
        time.sleep(3)
